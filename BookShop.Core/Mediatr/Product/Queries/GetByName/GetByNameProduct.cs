...
namespace BookShop.Core.Mediatr.Product.Queries.GetByName
{
    public static class GetByNameProduct
    {
        public record Query(string Name) : IRequest<IList<ProductModel>>;
        ...
        public class Handler : IRequestHandler<Query, IList<ProductModel>>
        {
            ...
            public async Task<IList<ProductModel>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                IList<Domain.Entities.Product> products = repository.GetByName(request.Name);
